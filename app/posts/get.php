<?php
    ...
    if(isset($_POST['getAll']) && $_POST['getAll']){
        $getPosts = $pdo->prepare('SELECT p.postID, p.title, p.postText, p.postTime, p.updateTime, p.image_url, u.nickname AS author FROM posts AS p, users AS u WHERE p.authorID=u.id');
        if(!$getPosts->execute()){
        ...
        $getPosts = $pdo->prepare('SELECT p.postID, p.title, p.postText, p.postTime, p.updateTime, p.image_url, u.nickname AS author FROM posts AS p, users AS u WHERE p.authorID=u.id ORDER BY p.postID DESC LIMIT 1');
        if(!$getPosts->execute()){
            echo json_encode(["error"=>true, "errorInfo"=>"Couldn't get posts."]);
            die;
        }
